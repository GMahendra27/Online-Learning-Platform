...
if (isset($_POST['login_submit'])) {
    ...
    $qry = "SELECT * FROM users WHERE username=? AND role=?";
...
if (isset($_POST['register_submit'])) {
    ...
    $password = password_hash($_POST['pwd_r'], PASSWORD_DEFAULT);
    $phoneNumber = $_POST['contact'];
    $role = $_POST['role_r']; 

    $qry0 = "SELECT * FROM users WHERE username=?";
    $stmt0 = $conn->prepare($qry0);
    ...
    if ($res0->num_rows == 1) {
        ...
        $qry = "INSERT INTO users (full_name, username, password, phone_number, role) VALUES (?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($qry);
        $stmt->bind_param("sssss", $fullname, $username, $password, $phoneNumber, $role);
        $res = $stmt->execute();
